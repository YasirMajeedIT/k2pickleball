...
<!-- Sidebar -->
<aside class="absolute left-0 top-0 z-[999] flex h-screen w-[280px] flex-col overflow-y-hidden bg-gradient-to-b from-surface-900 via-surface-900 to-surface-950 duration-300 ease-linear lg:static lg:translate-x-0 border-r border-surface-800/50"
    ...
    <div class="flex items-center justify-between gap-2 px-6 py-6">
        <a href="<?= ($baseUrl ?? '') . '/admin' ?>" class="flex items-center gap-3 group">
            ...
            <div>
                <span class="text-lg font-bold text-white tracking-tight">K2 Pickleball</span>
            </div>
        </a>
        <button x-on:click="sidebarOpen = false" class="block lg:hidden text-surface-400 hover:text-white rounded-lg p-1 hover:bg-white/5">
    ...
    <div class="flex flex-col overflow-y-auto sidebar-scroll flex-1 px-4 pb-4">
        <nav>
            <p class="mb-3 ml-3 text-[11px] font-semibold uppercase tracking-[0.15em] text-surface-500">Navigation</p>
            <ul class="flex flex-col gap-0.5">
                ...
                <?php
                    ...
                    $activeClass = $isActive
                        ? 'bg-primary-500/10 text-primary-400 border-primary-500/30 shadow-sm'
                        : 'text-surface-400 border-transparent hover:bg-white/[0.04] hover:text-surface-200';
                ?>
                <li>
                    <a href="<?= htmlspecialchars($item['url']) ?>"
                       class="<?= $activeClass ?> group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-medium border transition-all duration-200">
                        <svg class="w-[18px] h-[18px] flex-shrink-0 <?= $isActive ? 'text-primary-400' : 'text-surface-500 group-hover:text-surface-300' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <?= $icons[$item['icon']] ?? '' ?>
                        </svg>
                        <span><?= htmlspecialchars($item['label']) ?></span>
                        <?php if ($isActive): ?>
                        <span class="ml-auto h-1.5 w-1.5 rounded-full bg-primary-400"></span>
                        <?php endif; ?>
                    </a>
                </li>
                <?php endforeach; ?>
    ...
    <!-- Sidebar Footer -->
    <div class="border-t border-surface-800/50 px-4 py-4">
        <div class="flex items-center gap-3 rounded-xl bg-white/[0.04] px-3 py-3">
            <div class="h-9 w-9 rounded-lg bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-white font-bold text-xs shadow-sm flex-shrink-0">
                <?= htmlspecialchars(strtoupper(substr($org['name'] ?? 'K', 0, 2))) ?>
            </div>
            <div class="min-w-0">
                <p class="text-sm font-semibold text-white truncate"><?= htmlspecialchars($org['name'] ?? 'Organization') ?></p>
                <p class="text-[11px] text-surface-500 truncate"><?= htmlspecialchars($org['slug'] ?? 'Free plan') ?></p>
            </div>
        </div>
    </div>
</aside>
