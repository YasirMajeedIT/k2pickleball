        ...
        <div class="w-full max-w-lg">
            ...
            <div x-show="step===1" x-transition>
                ...
                <form @submit.prevent="nextStep()" class="mt-4 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="tel" x-model="form.phone" required placeholder="(555) 123-4567"
                            class="w-full px-4 py-3 rounded-xl bg-surface-900/50 border text-white placeholder-surface-500 focus:outline-none focus:ring-1 focus:ring-brand-500/30 transition-colors"
                            :class="fieldErrors.phone?'border-red-500/60':'border-surface-700/60 focus:border-brand-500/50'">
                        <p x-show="fieldErrors.phone" class="mt-1 text-xs text-red-400" x-text="fieldErrors.phone"></p>
                    </div>
            ...
            <div x-show="step===3" x-transition>
                ...
                <div x-show="selectedPlanPrice>0" x-transition class="mt-6 p-5 rounded-2xl border border-surface-700/60 bg-surface-900/30 space-y-4">
                    ...
                    <!-- Square combined card widget -->
                    <div id="sq-card-container" class="rounded-xl overflow-hidden min-h-[54px]"></div>
                ...
                <div class="mt-6 flex gap-3">
                    ...
                    <button @click="submit()" :disabled="loading" class="flex-1 flex items-center justify-center gap-2 px-6 py-3.5 font-semibold text-white bg-brand-600 hover:bg-brand-500 disabled:opacity-50 rounded-2xl shadow-lg shadow-brand-600/25 transition-all hover:-translate-y-0.5">
                        <span x-show="!loading" x-text="_googleToken ? (selectedPlanPrice>0 ? 'Pay & Complete Setup' : 'Complete Setup') : (selectedPlanPrice>0 ? 'Pay &amp; Create Account' : 'Create Free Account')"></span>
            ...
            <p class="mt-8 text-center text-xs text-surface-500">By creating an account, you agree to our <a :href="BASE_URL + '/terms'" class="hover:text-surface-300 underline">Terms</a> and <a :href="BASE_URL + '/privacy-policy'" class="hover:text-surface-300 underline">Privacy Policy</a>.</p>
        </div>
    </div>
</div>
...
if ($sqAppId): ?>
<script src="https://<?= $sqEnv === 'production' ? 'web' : 'sandbox.web' ?>.squarecdn.com/v1/square.js"></script>
<?php endif; ?>
...
function registerPage() {
    return {
        ...
        form: { first_name:'', last_name:'', email:'', password:'', password_confirmation:'', phone:'',
        ...
        async init() {
            ...
            this.renderGoogleBtn('google-signup-btn', 'signup_with');
        },
        ...
        renderGoogleBtn(elId, btnText) {
            ...
            // Initialize Google GSI only once, then render button
            this.waitForGoogle(() => {
                ...
                google.accounts.id.renderButton(el, { theme:'filled_black', size:'large', text: btnText, width: el.offsetWidth || 400 });
            });
        },
        ...
        nextStep() {
            ...
            this.step++;
            // Init Square card widget when reaching step 3
            if (this.step === 3 && SQ_APP_ID && SQ_LOC_ID && !this.sqCard) {
                this.$nextTick(() => this.initSquareCard());
            }
        },

        async initSquareCard() {
            // Wait for Square SDK to load (loaded async in <head>)
            for (let i = 0; i < 100; i++) {
                if (typeof Square !== 'undefined') break;
        ...
        async submit() {
            ...
            // Google user: already authenticated, go straight to org + subscription setup
            if (this._googleToken) {
                this.loadingMsg = 'Setting up your organization…';
                try {
            ...
            // Non-Google user: register, then redirect to verify email
            this.loadingMsg = 'Creating account…';

            // Tokenize card FIRST for paid plans (nonce needed after email verification)
            let paymentNonce = null;
            if (this.selectedPlanPrice > 0 && this.sqCard) {
                this.loadingMsg = 'Securing payment details…';
            ...
            try {
                ...
                sessionStorage.setItem('k2_pending_reg', JSON.stringify({
                    ...
                    plan_price:  this.selectedPlanPrice,
        ...
        async completeSetup(token, email) {
            ...
            const orgRes  = await fetch(BASE_URL + '/api/organizations', {
                ...
                body: JSON.stringify({
                    ...
                    address_line1: this.form.org_address || undefined
                })
            });
            ...
            await fetch(BASE_URL + '/api/subscriptions/create-for-org', {
                method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},
        ...
        async googleCallback(response) {
            ...
            try {
                ...
                // Save token and advance to org setup (step 2) instead of going straight to portal
                this._googleToken = d.access_token;
                if (d.user) {
                    this.form.first_name = d.user.first_name || this.form.first_name;
                    this.form.last_name  = d.user.last_name  || this.form.last_name;
                    this.form.email      = d.user.email      || this.form.email;
                }
                this.loading = false;
                this.step = 2;
