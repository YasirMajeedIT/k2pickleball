...
<head>
    ...
    <title>Reset Password — K2Pickleball.com</title>
...
<body class="bg-navy-950 text-white min-h-screen flex items-center justify-center px-6 py-12">
    ...
    <div class="relative w-full max-w-md" x-data="forgotPasswordForm()">
        <!-- Logo -->
        ...
        <div x-show="!sent">
            ...
            <form class="mt-8 space-y-5" @submit.prevent="submitForm()">
                <div>
                    ...
                    <input type="email" required autocomplete="email" x-model="email" class="w-full px-4 py-3 rounded-xl bg-navy-900/60 border border-navy-700/60 text-white placeholder-slate-500 focus:border-gold-500/40 focus:ring-1 focus:ring-gold-500/20 transition-colors text-sm" placeholder="you@example.com">
                </div>
        ...
        <div x-show="sent" x-cloak class="text-center">
            ...
            <button @click="sent = false; errorMsg = ''" class="mt-6 text-sm font-medium text-gold-500 hover:text-gold-400 transition-colors">Didn't receive it? Try again</button>
        </div>
