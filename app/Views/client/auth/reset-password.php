...
<head>
    ...
    <title>Set New Password — K2Pickleball.com</title>
...
<body class="bg-navy-950 text-white min-h-screen flex items-center justify-center px-6 py-12">
    ...
    <div class="relative w-full max-w-md" x-data="resetPasswordForm()">
        ...
        <!-- Error message -->
        <div x-show="errorMsg" x-cloak class="mb-4 p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm text-center" x-text="errorMsg"></div>

        <!-- Missing token warning -->
        <div x-show="!token" x-cloak class="text-center">
            <h1 class="font-display text-3xl font-extrabold text-white">Invalid Reset Link</h1>
            <p class="mt-3 text-sm text-slate-400">This password reset link is invalid or has expired. Please request a new one.</p>
            <a href="<?= $baseUrl ?>/forgot-password" class="mt-6 inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-bold text-navy-950 gradient-gold-bg rounded-xl shadow-lg shadow-gold-500/20 hover:shadow-gold-500/30 transition-all duration-300">
                Request New Link
            </a>
        </div>

        <div x-show="token && !done">
            ...
            <form class="mt-8 space-y-5" @submit.prevent="submitForm()">
                <div>
                    ...
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" required autocomplete="new-password" x-model="password" class="w-full px-4 py-3 rounded-xl bg-navy-900/60 border border-navy-700/60 text-white placeholder-slate-500 focus:border-gold-500/40 focus:ring-1 focus:ring-gold-500/20 transition-colors text-sm pr-11" placeholder="Minimum 8 characters">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input :type="showConfirm ? 'text' : 'password'" required autocomplete="new-password" x-model="passwordConfirmation" class="w-full px-4 py-3 rounded-xl bg-navy-900/60 border border-navy-700/60 text-white placeholder-slate-500 focus:border-gold-500/40 focus:ring-1 focus:ring-gold-500/20 transition-colors text-sm pr-11" placeholder="Re-enter password">
            ...
            async submitForm() {
                ...
                try {
                    ...
                    if (json.status === 'success') {
                        ...
                        this.errorMsg = json.message || 'Failed to reset password. The link may have expired.';
                    }
                } catch (e) {
