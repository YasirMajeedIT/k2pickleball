...
$breadcrumbs = [['label' => 'Membership Plans', 'url' => ($baseUrl ?? '') . '/admin/membership-plans'], ['label' => 'Edit']];
$planId = $id ?? '';
$apiBase = $baseUrl ?? '';

ob_start();
?>
<div x-data="membershipPlanEdit()" x-init="init()" class="mx-auto max-w-4xl space-y-6">
    <!-- Loading -->
    ...
    <form x-show="!loading" @submit.prevent="submitForm()">
        <!-- Plan Details -->
        <div class="rounded-2xl border border-surface-200 bg-white dark:border-surface-800 dark:bg-surface-900 shadow-soft overflow-hidden mb-6">
            <div class="border-b border-surface-100 dark:border-surface-800 px-6 py-5 bg-surface-50/50 dark:bg-surface-800/30">
                ...
                <p class="text-sm text-surface-400 mt-0.5">Update the membership plan details and pricing</p>
            </div>
            <div class="space-y-5 px-6 py-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-surface-700 dark:text-surface-300">Facility</label>
                        <select x-model="form.facility_id" class="w-full rounded-xl border border-surface-200 bg-white px-4 py-3 text-sm shadow-soft focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 dark:border-surface-700 dark:bg-surface-800 dark:text-white">
                            <option value="">Select Facility...</option>
                            <template x-for="f in facilities" :key="f.id">
                                <option :value="f.id" x-text="f.name" :selected="f.id == form.facility_id"></option>
                            </template>
                        </select>
                    </div>
                </div>
                <div>
                    ...
                    <textarea x-model="form.description" rows="3" placeholder="Plan description..." class="w-full rounded-xl border border-surface-200 bg-white px-4 py-3 text-sm shadow-soft focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 dark:border-surface-700 dark:bg-surface-800 dark:text-white"></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        ...
                        <input type="number" x-model="form.price" required step="0.01" min="0" class="w-full rounded-xl border border-surface-200 bg-white px-4 py-3 text-sm shadow-soft focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 dark:border-surface-700 dark:bg-surface-800 dark:text-white">
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-surface-700 dark:text-surface-300">Setup Fee ($)</label>
                        <input type="number" x-model="form.setup_fee" step="0.01" min="0" class="w-full rounded-xl border border-surface-200 bg-white px-4 py-3 text-sm shadow-soft focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 dark:border-surface-700 dark:bg-surface-800 dark:text-white">
                    </div>
                    <div>
                    ...
                    <div class="flex items-end pb-1 gap-6">
                        ...
                        <label class="inline-flex items-center gap-3 cursor-pointer group">
                            <div class="relative">
                                ...
                                <div class="h-5 w-9 rounded-full bg-surface-200 peer-checked:bg-green-500 dark:bg-surface-700 transition-colors"></div>
                                <div class="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transform peer-checked:translate-x-4 transition-transform"></div>
                            </div>
                            <span class="text-sm font-medium text-surface-600 dark:text-surface-400">Active</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div class="rounded-2xl border border-surface-200 bg-white dark:border-surface-800 dark:bg-surface-900 shadow-soft overflow-hidden mb-6">
            <div class="border-b border-surface-100 dark:border-surface-800 px-6 py-5 bg-surface-50/50 dark:bg-surface-800/30">
                ...
                <p class="text-sm text-surface-400 mt-0.5">Select which categories are included or discounted</p>
            </div>
            <div class="px-6 py-6">
                <template x-if="categories.length === 0">
                    <p class="text-sm text-surface-400 italic">No categories found.</p>
                </template>
                <div class="space-y-3">
        ...
        <div class="rounded-2xl border border-surface-200 bg-white dark:border-surface-800 dark:bg-surface-900 shadow-soft overflow-hidden mb-6">
            ...
            <div class="px-6 py-6">
                <template x-if="sessionTypes.length === 0">
                    <p class="text-sm text-surface-400 italic">No session types found.</p>
                </template>
                <div class="space-y-3">
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit" :disabled="submitting" class="rounded-xl bg-gradient-to-r from-primary-600 to-primary-700 px-6 py-2.5 text-sm font-semibold text-white hover:from-primary-700 hover:to-primary-800 disabled:opacity-50 shadow-soft transition-all">
                <span x-show="!submitting">Update Membership Plan</span>
...
function membershipPlanEdit() {
    return {
        ...
        async loadPlan() {
            try {
                const res = await authFetch(APP_BASE + '/api/membership-plans/<?= $planId ?>');
                const json = await res.json();
                if (json.data) {
                    const d = json.data;
                    this.form.name = d.name || '';
                    this.form.facility_id = d.facility_id || '';
                    this.form.description = d.description || '';
                    this.form.duration_type = d.duration_type || 'monthly';
                    this.form.duration_value = d.duration_value || '';
                    this.form.price = d.price || '';
                    this.form.setup_fee = d.setup_fee || '0';
                    this.form.renewal_type = d.renewal_type || 'auto';
                    this.form.is_taxable = d.is_taxable == 1;
                    this.form.is_active = d.is_active == 1;
                    this.form.color = d.color || '#6366f1';
                    this.form.max_members = d.max_members || '';
                    // Populate benefits
                    if (d.included_categories) this.includedCategories = d.included_categories.map(c => String(c.category_id));
                    if (d.discounted_categories) this.discountedCategories = d.discounted_categories.map(c => String(c.category_id));
                    if (d.included_session_types) this.includedSessionTypes = d.included_session_types.map(s => String(s.session_type_id));
                    if (d.discounted_session_types) this.discountedSessionTypes = d.discounted_session_types.map(s => String(s.session_type_id));
                    // Set discount percentages after next tick
                    this.$nextTick(() => {
                        (d.discounted_categories || []).forEach(c => {
                            const el = document.querySelector(`[data-cat-discount="${c.category_id}"]`);
                            if (el) el.value = c.discount_percentage || '';
                        });
                        (d.discounted_session_types || []).forEach(s => {
                            const el = document.querySelector(`[data-st-discount="${s.session_type_id}"]`);
                            if (el) el.value = s.discount_percentage || '';
                        });
                    });
                }
        ...
        async submitForm() {
            ...
            try {
                ...
                const res = await authFetch(APP_BASE + '/api/membership-plans/<?= $planId ?>', { method: 'PUT', body });
                const json = await res.json();
                if (res.ok) {
                    window.dispatchEvent(new CustomEvent('toast', { detail: { message: 'Membership plan updated', type: 'success' } }));
