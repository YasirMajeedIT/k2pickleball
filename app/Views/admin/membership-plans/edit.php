            ...
            <div class="space-y-5 px-6 py-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select x-model="form.facility_id" @change="loadSessionTypes()" class="w-full rounded-xl border border-surface-200 bg-white px-4 py-3 text-sm shadow-soft focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 dark:border-surface-700 dark:bg-surface-800 dark:text-white">
...
function membershipPlanEdit() {
    return {
        ...
        async loadSessionTypes(preserveBenefits) {
            this.sessionTypes = [];
            if (!preserveBenefits) this.stBenefits = {};
            ...
            try {
                ...
                if (this.pendingStBenefits) {
                    this.stBenefits = this.pendingStBenefits;
                    this.pendingStBenefits = null;
                }
            } catch (e) { console.error(e); }
        },
        ...
        async loadPlan() {
            try {
                ...
                if (json.data) {
                    ...
                    this.form.renewal_price = d.renewal_price || '';
                    this.form.renewal_price_policy = d.renewal_price_policy || 'current_price';
                    ...
                    // Populate category benefits
                    (d.included_categories || []).forEach(c => {
                        this.catBenefits[c.category_id] = {
                            type: 'included',
                            price: c.price || '',
                            usage_limit: c.usage_limit || '',
                            usage_period: c.usage_period || 'unlimited',
                            discount_percentage: ''
                        };
                    });
                    (d.discounted_categories || []).forEach(c => {
                        this.catBenefits[c.category_id] = {
                            type: 'discounted',
                            price: '', usage_limit: '', usage_period: 'unlimited',
                            discount_percentage: c.discount_percentage || ''
                        };
                    });

                    // Prepare session type benefits (will be applied after loadSessionTypes)
                    const stb = {};
                    (d.included_session_types || []).forEach(s => {
                        stb[s.session_type_id] = {
                            type: 'included',
                            price: s.price || '',
                            usage_limit: s.usage_limit || '',
                            usage_period: s.usage_period || 'unlimited',
                            discount_percentage: ''
                        };
                    });
                    ...
                    this.pendingStBenefits = stb;

                    // Load session types for the facility
                    await this.loadSessionTypes(true);
                }
