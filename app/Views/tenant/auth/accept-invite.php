<?php
/**
 * Tenant Auth — Accept Invitation (Standalone, no layout)
 * K2 Navy/Gold Theme. POSTs to /api/auth/accept-invite. Expects ?token=X in URL.
 */
...
<head>
    ...
    <title>Accept Invitation — <?= $orgName ?></title>
...
<body class="bg-navy-950 text-white min-h-screen flex items-center justify-center p-6">
    ...
    <div class="w-full max-w-md" x-data="acceptInvitePage()">
        <div class="text-center mb-8">
            ...
            <?php endif; ?>
            <h1 class="text-2xl font-display font-extrabold text-white">Welcome to <?= $orgName ?></h1>
            <p class="text-slate-400 text-sm mt-2">Set your password to activate your account.</p>
        </div>
        ...
        <div x-show="done" x-transition class="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-8 text-center">
            ...
            <p class="text-white font-bold mb-1">Account Activated!</p>
            <p class="text-slate-400 text-sm">Your password has been set and your account is ready. You can now sign in.</p>
        ...
        <!-- Invalid token -->
        <div x-show="!token && !done" class="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 text-center">
            ...
            <p class="text-white font-bold mb-1">Invalid Invitation Link</p>
            <p class="text-slate-400 text-sm">This invitation link is missing or malformed. Please check the link from your email or ask your administrator to resend the invitation.</p>
        </div>

        <!-- Form -->
        <form x-show="token && !done" @submit.prevent="submit()" class="space-y-5">
            <div>
            ...
            <button type="submit" :disabled="loading" class="w-full py-3.5 rounded-xl font-bold text-sm gradient-gold-bg text-navy-950 hover:shadow-lg hover:shadow-gold-500/20 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                <span x-text="loading ? 'Activating...' : 'Set Password & Activate'"></span>
            </button>
    ...
    function acceptInvitePage() {
        return {
            ...
            async submit() {
                ...
                if (!this.token) { this.error = 'Invalid or missing invitation token.'; return; }
                this.loading = true;
                try {
                    const resp = await fetch(baseApi + '/api/auth/accept-invite', {
                        method: 'POST',
                    ...
                    else { this.error = json.message || 'Failed to activate your account.'; }
