...
final class MembershipPlanController extends Controller
{
    ...
    public function store(Request $request): Response
    {
        $data = Validator::validate($request->all(), [
            ...
            'renewal_price'         => 'nullable|numeric',
            'renewal_price_policy'  => 'nullable|in:current_price,locked_price',
        ...
        $planData = [
            ...
            'renewal_price'        => isset($data['renewal_price']) && $data['renewal_price'] !== '' ? round((float) $data['renewal_price'], 2) : null,
            'setup_fee'            => round((float) ($data['setup_fee'] ?? 0), 2),
            'renewal_type'         => $data['renewal_type'] ?? 'auto',
            'renewal_price_policy' => $data['renewal_price_policy'] ?? 'current_price',
    ...
    public function assignMembership(Request $request): Response
    {
        ...
        $membershipData = [
            ...
            'locked_price'       => round((float) $plan['price'], 2),
            'auto_renew'         => ($plan['renewal_type'] ?? 'auto') !== 'none' ? 1 : 0,
